    ...
    if request.method == 'POST':  # если это запрос к api от фронта
        ...
        if request.data.get('oper') == 'user_exist':
            ...
            if str(request.data.get('num')) == '-1':
                ...
                s = open(f'C:/inetpub/wwwroot/backend_api/media/img/{folder}/' + f'photo_{str(request.data.get('num'))}.png',
                         'rb').read()
            ...
            slov = {'answer': 1, 'description': arr[-2].split('\n'), 'name': arr[1], 'brand': arr[3],
            ...
            if ask=='':
                try:
                    if int(type) == 1:
                        return Response({'ids': sql.execute(f"SELECT id FROM items WHERE price BETWEEN {cost_min} AND {cost_max} ORDER BY price").fetchall()})
                    elif int(type) == -1:
                        return Response({'ids': sql.execute(f"SELECT id FROM items WHERE price BETWEEN {cost_min} AND {cost_max} ORDER BY price DESC").fetchall()})
                    else:
                        return Response(
                            {'ids': sql.execute(f"SELECT id FROM items WHERE price BETWEEN {cost_min} AND '{cost_max}' ORDER BY popular DESC").fetchall()})
                except:
                    return Response({'ids':'-1'})
            else:
                try:
                    if int(type) == 1:
                        asad = sql.execute(f"SELECT id FROM items WHERE description LIKE '%{ask}%' AND price BETWEEN {cost_min} AND {cost_max} ORDER BY price").fetchall()
                        return Response({'ids':'-1' if len(asad)==0 else asad})
                    elif int(type) == -1:
                        asad = sql.execute(f"SELECT id FROM items WHERE description LIKE '%{ask}%' AND price BETWEEN {cost_min} AND {cost_max} ORDER BY price DESC").fetchall()
                        return Response({'ids':'-1' if len(asad)==0 else asad})
                    else:
                        asad = sql.execute(f"SELECT id FROM items WHERE description LIKE '%{ask}%' AND price BETWEEN {cost_min} AND {cost_max} ORDER BY popular DESC").fetchall()
                        return Response({'ids':'-1' if len(asad)==0 else asad})
                except:
                    return Response({'ids':'-1'})
            ...
            db.commit()
            text = f"Привет! твой код для восстановления пароля - {code}"
...
"""
            ...
            return Response(
                {'mail': arr[1], 'name': arr[2], 'phone': arr[5], 'address': arr[6],
                 'postcode': arr[7], 'date': arr[9], 'status': arr[10],'busket':bsk})
